...
export class RegisterComponent implements OnInit {
  ...
  crearUsuario() {
    ...
    console.log(body)
    this.usuarioService.postUsuario(body).subscribe(
      res => {
        // Si el servidor devuelve una respuesta exitosa (por ejemplo, un código 200):
        this.mensajeExito()
        this.router.navigateByUrl('/login');

      },
      err => {
        // Si el servidor devuelve un error (por ejemplo, un código 404):        
        this.mensajeError()
      }
    );
  }
  ...
  async mensajeError() {
    const alert = await this.alertController.create({
      header: 'El usuario no pudo ser creado.',
